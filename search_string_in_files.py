    ...
    def __search(self, file_):
        ...
        try:
            ...
            if _dict:
                ...
                print "[FILE]\t{0}\n".format(self.color_obj.warn(file_.replace(os.getenv("HOME"), "~")))
                self.color_obj.err("[SEARCH STRING]\t{0} --> [Case insensitive Search...]\n".format(self.string))
                ##print '''Found "{0}"'''.format(self.string)
                for line in sorted(_dict.keys()):
                    self.color_obj.info("[LINE] {0}\t{1}".format(line, _dict[line].strip()))
                print "-"*80
